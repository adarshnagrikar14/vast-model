...
def prepare_latent_image_ids_(height, width, device, dtype):
    ...
    latent_image_ids[..., 1] = latent_image_ids[..., 1] + \
        torch.arange(height//2, device=device)[:, None]  # y
    latent_image_ids[..., 2] = latent_image_ids[..., 2] + \
        torch.arange(width//2, device=device)[None, :]   # x
...
def prepare_latent_subject_ids(height, width, device, dtype):
    latent_image_ids = torch.zeros(
        height // 2, width // 2, 3, device=device, dtype=dtype)
    latent_image_ids[..., 1] = latent_image_ids[..., 1] + \
        torch.arange(height // 2, device=device)[:, None]
    latent_image_ids[..., 2] = latent_image_ids[..., 2] + \
        torch.arange(width // 2, device=device)[None, :]
...
def resize_position_encoding(batch_size, original_height, original_width, target_height, target_width, device, dtype):
    ...
    latent_image_ids_resized[..., 1] = latent_image_ids_resized[..., 1] + \
        torch.arange(target_height//2, device=device)[:, None] * scale_h
    latent_image_ids_resized[..., 2] = latent_image_ids_resized[..., 2] + \
    ...
    cond_latent_image_ids = latent_image_ids_resized.reshape(
        ...
        cond_latent_image_id_width, cond_latent_image_id_channels
    )
...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def _get_t5_prompt_embeds(
        ...
        prompt_embeds = self.text_encoder_2(
            text_input_ids.to(device), output_hidden_states=False)[0]
    ...
    def prepare_latents(
        ...
        if subject_image is not None:
            shape_subject = (batch_size, num_channels_latents,
            ...
            subject_latents = self._pack_latents(
                subject_image_latents, batch_size, num_channels_latents, height_cond*sub_number, width_cond)
            mask2 = torch.zeros(shape_subject, device=device, dtype=dtype)
            mask2 = self._pack_latents(
                mask2, batch_size, num_channels_latents, height_cond*sub_number, width_cond)
        ...
        if condition_image is not None:
            shape_cond = (batch_size, num_channels_latents,
            ...
            cond_latents = self._pack_latents(
                image_latents, batch_size, num_channels_latents, height_cond*cond_number, width_cond)
            mask3 = torch.zeros(shape_cond, device=device, dtype=dtype)
            mask3 = self._pack_latents(
                mask3, batch_size, num_channels_latents, height_cond*cond_number, width_cond)
    ...
    def __call__(
        ...
        if sub_number > 0:
            ...
            for subject_image in subject_images:
                ...
                subject_image = pad(
                    subject_image,
                    padding=(int(pad_w / 2), int(pad_h / 2),
                             int(pad_w / 2), int(pad_h / 2)),
                    fill=0
                )
        ...
        lora_scale = (
            self.joint_attention_kwargs.get(
        ...
        if output_type == "latent":
            ...
            image = self.vae.decode(latents.to(
                dtype=self.vae.dtype), return_dict=False)[0]
